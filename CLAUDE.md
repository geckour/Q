...
リリースノートは既存のノートを土台にして作る。未公開のリリース（GitHub Release はあるが Play Store に出ていないもの、プレリリースを含む）がある場合は、そのノートの項目をすべて引き継いだうえで、新しい項目を追記する。前回の公開以降に入った変更がユーザーから見て漏れないようにするため。
